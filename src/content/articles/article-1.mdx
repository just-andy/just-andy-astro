---
title: Roadtesting your DS with a UI workbench
description: Description
summary: How to use a workbench file to stress test UI decisions
keywords: ["Blog", "Article"]
publishDate: 2023-06-15 15:14:39.004
---

import {Image} from "astro:assets";
import banner from "../../images/projects/cards-of-death/cod-banner.jpg";

## Heading 2

<Image src={banner} width={1024} alt="banner" class="full-width" />

How Can I Use Lorem Ipsum Tool For My Website?
You can click on the 'item to generate' column and select the format you want content in.

<section class="full-width">
Below that, you can select if you want an HTML tag in your content or not

After that, you can choose how many paragraphs you want in the 'how many items to generate' column.

Then, you can choose the minimum and maximum words you want per sentence.
</section>

```
<div>
<h1>testing this</h1>
</div>
```

<section class="full-width text-section-invert bg-section-invert padding-block-10 content-grid">
Below that, you can select if you want an HTML tag in your content or not

After that, you can choose how many paragraphs you want in the 'how many items to generate' column. Then, you can choose the minimum and maximum words you want per sentence.
</section>


<section class="breakout">
Later, you can select the minimum and maximum sentences you want per paragraph.

Finally, click on the button 'generate'

Taddalaa!
</section>

You got your Lorem Ipsum content for your website.